    ...
    public void parse(String path) throws ConfigException {
        ...
        try {
            ...
            try {
                ...
                /**
                 * 将我们上面的InputStream流取出出来 解析成键值对 然后存放到数据中。
                 * 等于说把我们配置在coo.cfg中的文件信息全部读取出来并以Key-Value的形式存放在cfg对象中
                 */
                cfg.load(in);
            } finally {
                in.close();
            ...
            /**
             * 解析Properties对象里面的键值对
             * 然后将信息存入当前的这个类中，需要解析的内容有很多 ，你在配置文件里面配置了什么
             * 这边就会处理什么东西  比如会处理Zookeeper的集群信息。
             */
            parseProperties(cfg);
        } catch (IOException e) {
        ...
        for (Entry<Object, Object> entry : zkProp.entrySet()) {
            ...
            if (key.equals("dataDir")) {
                ...
                if (type == LearnerType.OBSERVER) {
                    // 如果是OBSERVER就会放到observers容器中
                    // 这一点说明 非观察者和观察者的容器不在同一个地方
                    // 所以Zookeeper的过半机制可能就是和这一个有关？？？ 后面会证明。
