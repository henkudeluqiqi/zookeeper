...
public class ClientCnxn {
    ...
    class SendThread extends ZooKeeperThread {
        ...
        void readResponse(ByteBuffer incomingBuffer) throws IOException {
            ...
            synchronized (pendingQueue) {
                // 如果pendingQueue中没有数据就直接抛出异常
                if (pendingQueue.size() == 0) {
                    throw new IOException("Nothing in the queue, but got "
                            + replyHdr.getXid());
                }
                // 有的话就拿出拿出第一个数据并且删除第一个数据
             ...
             */
            try {
                // 判断发送出去的Xid和放回的Xid是否一致 如果不一致就会抛出异常
            ...
            while (state.isAlive()) {
                try {
                    ...
                    if (!clientCnxnSocket.isConnected()) {
                        ...
                        /**
                         * 判断连接是否关闭 如果连接关闭那么就直接退出
                         * 第一次进来closing 恒定是false
                         */
                        ...
                        if (rwServerAddress != null) {
                            ...
                            // 所以会进到这个方法
                            // 如果你连接失败了 那么zookeeper会重新尝试进行连接 ， hostProvider中保存了我们配置的所有地址 因为前面对我们的地址进行了解析
                            // 如果有多个连接地址且第一个地址连接失败了 就会去尝试连接下一个地址
                            serverAddress = hostProvider.next(1000);
                        }

                        // 然后我们就取得了一个连接的地址对象 -> serverAddress
                        // 并且开始连接
                    ...
                    // 如果超时了 zookeeper会进行抛出异常
                    // 由于这里是try cache  所以 zookeeper的重连机制就是在这里实现的
                    if (to <= 0) {
                        String warnInfo;
                    ...
                    if (state.isConnected()) {
                        ...
                        if (timeToNextPing <= 0 || clientCnxnSocket.getIdleSend() > MAX_SEND_PING_INTERVAL) {
                            // 会发出PING的命令 PING的命令和Create命令差不多的逻辑
                            sendPing();
                    ...
                    // 当连接成功的时候就可以出来我们outgoingQueue中的数据了
                    // outgoingQueue队列中存放了我们需要发出的命令
    ...
    public ReplyHeader submitRequest(RequestHeader h, Record request,
        ...
        // 打包并没有发送 ，只是将需要发送的命令添加到 outgoingQueue 中
        // zookeeper中会一直读取outgoingQueue中的数据 然后发送命令
